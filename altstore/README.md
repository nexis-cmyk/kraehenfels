...
Der Feed liegt als [`source.json`](source.json) im Repository und ist auf den iPad-Release `v4.3.0` ausgelegt. Der macOS-Release-Workflow erzeugt bei weiteren Versionen zusätzlich eine ausgefüllte `source.generated.json` mit Größe und SHA-256.

In AltStore Classic: `Sources` öffnen, `+` wählen und diese Feed-URL eintragen:

Aktueller Release-Feed (für die Installation von v4.3.0):
